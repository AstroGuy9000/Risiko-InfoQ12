...
public abstract class TextFieldCust extends Widget implements Disableable {
	...
	public TextFieldCust (@Null String text, Skin skin, KeyBinds binds) {
		this(text, skin.get(TextFieldStyle.class), binds);
	}

	public TextFieldCust (@Null String text, Skin skin, String styleName, KeyBinds binds) {
		this(text, skin.get(styleName, TextFieldStyle.class), binds);
	}

	public TextFieldCust (@Null String text, TextFieldStyle style, KeyBinds bindsIn) {
		setStyle(style);
	...
	public void next (boolean up) {
		...
		while (true) {
			TextFieldCust textField = current.findNextTextField(stage.getActors(), null, bestCoords, currentCoords, up);
	...
	/** @return May be null. */
	private @Null TextFieldCust findNextTextField (Array<Actor> actors, @Null TextFieldCust best, Vector2 bestCoords,
		Vector2 currentCoords, boolean up) {
		for (int i = 0, n = actors.size; i < n; i++) {
			Actor actor = actors.get(i);
			if (actor instanceof TextFieldCust) {
				if (actor == this) continue;
				TextFieldCust textField = (TextFieldCust)actor;
	...
	static public interface TextFieldListener {
		public void keyTyped (TextFieldCust textField, char c);
	}

	/** Interface for filtering characters entered into the text field.
	 * @author mzechner */
	static public interface TextFieldFilter {
		public boolean acceptChar (TextFieldCust textField, char c);
	...
	/** The default {@link OnscreenKeyboard} used by all {@link TextFieldCust} instances. Just uses
	...
	public class TextFieldClickListener extends ClickListener {
		...
		public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
			...
			Stage stage = getStage();
			if (stage != null) stage.setKeyboardFocus(TextFieldCust.this);
			keyboard.show(true);
			hasSelection = true;
			return true;
		}
		...
		/** Checks if focus traversal should be triggered. The default implementation uses {@link TextFieldCust#focusTraversal} and the
		 * typed character, depending on the OS.
		 * @param character The character that triggered a possible focus traversal.
		 * @return true if the focus should change to the {@link TextFieldCust#next(boolean) next} input field. */
		...
		public boolean keyTyped (InputEvent event, char character) {
			...
			if (checkFocusTraversal(character))
				...
				if (add || remove) {
					...
					if (add && !remove) {
						...
						if (!enter && filter != null && !filter.acceptChar(TextFieldCust.this, character)) return true;
			...
			if (listener != null) listener.keyTyped(TextFieldCust.this, character);
			
			thingTyped(character);
