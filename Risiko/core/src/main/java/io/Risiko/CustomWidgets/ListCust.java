...
package io.Risiko.CustomWidgets;
 ...
 * @author mzechner
 * @author Nathan Sweet */
public class ListCust<T> extends Widget implements Cullable {
	ListStyle style;
	...
	public ListCust (Skin skin, KeyBinds bindsIn) {
		this(skin.get(ListStyle.class), bindsIn);
	}

	public ListCust (Skin skin, String styleName, KeyBinds bindsIn) {
		this(skin.get(styleName, ListStyle.class), bindsIn);
	}

	public ListCust (ListStyle style, KeyBinds bindsIn) {
		...
		addListener(keyListener = new InputListener() {
			...
			public boolean keyTyped (InputEvent event, char character) {
				...
				for (int i = 0, n = items.size; i < n; i++) {
					if (ListCust.this.toString(items.get(i)).toLowerCase().startsWith(prefix)) {
						setSelectedIndex(i);
						break;
					}
				}
		...
		addListener(new InputListener() {
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				...
				if (getStage() != null) getStage().setKeyboardFocus(ListCust.this);
	...
	public void setBinds(KeyBinds bindsIn) {
		binds = bindsIn;
	}
	...
	public int getItemIndexAt (float y) {
		...
		Drawable background = ListCust.this.style.background;
		if (background != null) {
	...
	/** The style for a list, see {@link ListCust}.
	 * @author mzechner
	 * @author Nathan Sweet */
	// Hab ich gelöscht btw
}